...
class Storage extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->table->set_heading('ID', 'Номер Накладной', 'Производитель', 'Количество', 'Вес', 'Материал', 'Дата', 'Коментарий', 'Сумма','Ширина','Высота','Глубина' ,"Действие");
        $temp = $this->storagemodel->getAllStorage();
        $count = 0;
        foreach ($temp as $value) {
            $data['storage'][] = $value;
            $data['storage'][$count]['action'] = " <div class='btn btn-primary' onclick='getStorage(" . $value['id_storage'] . ")'><i class='fa fa-pencil'></i> </div>
